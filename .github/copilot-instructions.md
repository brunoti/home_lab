...
- **Interactive Command Prohibition:** NEVER execute commands that open interactive sessions you cannot control (editors, prompts, REPLs). These freeze the terminal and require manual intervention.
  - **Forbidden Examples:** `git rebase --continue` (opens editor), `git commit` without `-m`, `python` (REPL), `vim/nano/emacs` directly
  - **Safe Alternatives:** Use non-interactive flags (`git commit -m "message"`, `GIT_EDITOR=true git rebase --continue`) or ask user to run the command
...
**Workspace patterns:** Project directories (active work, git repos), Documentation (notes, guides, `.md` with date-based naming), Temporary (`tmp/`, clean up after), Configuration, Credentials (`.env`, config files).
